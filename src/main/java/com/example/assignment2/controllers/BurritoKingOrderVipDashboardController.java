...
public class BurritoKingOrderVipDashboardController extends CommonFunctions{
    ...
    @FXML
    private Button mealPlus;

    @FXML
    private Button mealMinus;
    ...
    // Increments the soda count and updates the label, disabling buttons as needed.
    @FXML
    protected void incrementMeal() {
        int count = Integer.parseInt(mealCount.getText());
        count++;
        mealCount.setText(String.valueOf(count));
        disablePlusButton(mealPlus, mealCount);
        disableMinusButton(mealMinus, mealCount);
    }

    // Decrements the soda count and updates the label, disabling buttons as needed.
    @FXML
    protected void decrementMeal() {
        int count = Integer.parseInt(mealCount.getText());
        count--;
        mealCount.setText(String.valueOf(count));
        disablePlusButton(mealPlus, mealCount);
    ...
    private void setCounts(boolean isUpdateCart, List<Food> food){
        if (!isUpdateCart){
            ...
            for (Food food1 : food){
                ...
                } else if (food1.getFoodType().equals(FoodType.MEAL)){
                    mealCount.setText(String.valueOf(food1.getQuantity()));
                } else {
    ...
    protected void addOrderToDatabase() throws IOException {
        ...
        int mealCounter = Integer.parseInt(mealCount.getText());

        Pair<Double, String> paymentInfo = BurritoKingApplication.getMenu().order(burritoCounter, friesCounter, sodaCounter, mealCounter);
